...
function selectSize({ sizes }) {
    ...
    return (
        sizes.map((size, index) => {
            return (
                <label>
                    <input class="sr-only peer" name="size" type="radio" value="{size.label}" onClick={handleClick(size.label)} />
...
export default function Page({ product }) {
    const [selectedSize] = React.useState();
        ...
        <div class="font-sans font">
            <nav class="flex items-center justify-between flex-wrap header-bg p-6">
                <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto justify-end">
                    <div>
                        <a href="#" class="inline-block text-sm px-4 py-2 leading-none border rounded text-gray-400  hover: mt-4 lg:mt-0">My Cart</a>
                    </div>
                </div>
            </nav>
            <div class="flex-none w-48 relative">
                <img src="/classic-tee.jpg" alt="" class="absolute inset-0 w-full h-full object-cover" loading="lazy" />
            </div>
            <form class="flex-auto p-6">
                ...
                <div class="flex items-baseline mt-4 mb-6 pb-6">
                    <div class="space-x-2 flex text-sm">
                        <selectSize sizes={product.sizeOptions} onClick={handleClick}/>
                    </div>
                </div>
                <div class="flex space-x-4 mb-6 text-sm font-medium">
                    <div class="flex-auto flex space-x-4">
                        <button class="h-10 px-6 font-semibold border border-slate-900 border-2 text-slate-900" type="button">
                            Add to Cart
                        </button>
                    </div>
                </div>
